...
This script computes the Estimated Process Downloads KPI from:
  - Public repository download events
  - Validated UNIT_PROCESS counts
  - LCACS public browse PROCESS totals

Designed to be executed directly or orchestrated by the KPI framework.
Runtime parameters (index names, ES URL, etc.) are supplied via command-line arguments.
"""
        ...
        if total_count > 0:
            if unit_count > 0:
                if total_count >= unit_count:
                    ...
                    note = (
                        "Browse PROCESS total was lower than validated UNIT_PROCESS count. "
                        "Using validated UNIT_PROCESS count as total. "
                        "LCI_RESULT/System Process count set to 0 because the browse total "
                        "could not safely be reconciled."
                    )
            else:
